import {
  ...
  const OrderSummary = ({ cartItems = [], shippingData = {}, paymentData = {} }) => {
    const getMissingInformation = () => {
      ...
      } else if (!shippingData.firstName || !shippingData.lastName || !shippingData.address) {
        missing.push('Incomplete shipping information');
      }
  
      if (!paymentData || !Object.keys(paymentData).length) {
        missing.push('Payment information is missing');
      } else if (!paymentData.cardName || !paymentData.cardNumber || !paymentData.expiryDate) {
        missing.push('Incomplete payment information');
      }
